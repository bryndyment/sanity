...
class Query {
  ...
  orderBy(field) {
    ...
    if (fieldType !== 'string') {
      throw new Error("orderBy() takes a field name as first argument, ".concat(fieldType, " given"));
    }
  ...
  clone() {
    return new Query().to(this._to).from(this._from).constraint(this._constraints).orderBy(this._orderBy).params(this._params);
  }
